...
class ConditionPairingTest extends ParserTestCase
{
    public function testBasicConditionPairing()
    {
        $template = <<<'EOT'
@if ($something == true)
...
EOT;
    ...
    public function testElseIfElsePairing()
    {
        $template = <<<'EOT'
@if ($something == true)
...
EOT;
    ...
    public function testNestedConditionPairing()
    {
        $template = <<<'EOT'
...
EOT;
        ...
        $d2 = $directives[1];   // @if
        $d3 = $directives[2];   // @if
        $d4 = $directives[3];   // @elseif
        $d5 = $directives[4];   // @else
        $d6 = $directives[5];   // @endif
        $d7 = $directives[6];   // @elseif
        $d8 = $directives[7];   // @else
        $d9 = $directives[8];   // @endif
        $d10 = $directives[9];  // @elseif
        $d11 = $directives[10]; // @else
        $d12 = $directives[11]; // @endif
    ...
    public function testCustomConditionPairing()
    {
...
EOT;
        ...
        $d1 = $directives[0]; // @disk
        $d2 = $directives[1]; // @elsedisk
        $d3 = $directives[2]; // @else
        $d4 = $directives[3]; // @enddisk
    ...
    public function testDefaultSpeculativeConditions()
    {
...
EOT;
    ...
    /**
     * @dataProvider conditionalDirectives
     */
    public function testAmbiguousConditionDirectivePairing($open, $close)
    {
...
EOT;
    ...
    public function testUnpairedConditionsDoNotCauseInfiniteLoop()
    {
        $template = <<<'EOT'
...
EOT;
        ...
        $this->assertNull($directives[0]->isClosedBy);
        $this->assertNull($directives[1]->isOpenedBy);
        $this->assertNull($directives[1]->isClosedBy);
        $this->assertDirectivesArePaired($directives[2], $directives[3]);
    }

    public function testUnpairedConditionsDoNotCauseInfiniteLoopTwo()
    {
        $template = <<<'EOT'
@if   ($something) 
    <p>one</p>
...
EOT;
    ...
    public function testUnpairedConditionsDoNotCauseInfiniteLoopThree()
    {
        $template = <<<'EOT'
...
EOT;
    ...
    public function testUnpairedConditionsDoNotCauseInfiniteLoopFour()
    {
        $template = <<<'EOT'
@if (Route::has('login'))
A
...
EOT;
    ...
    public function testMixedConditionsArePaired()
    {
        $template = <<<'EOT'
...
EOT;
    ...
    public static function conditionalDirectives()
    {
        return [
            ['unless', 'endunless'],
            ['env', 'endEnv'],
